...
/**
 *
 * Adapted from https://github.com/mike-gimelfarb/numerical-integration, which is based on QUADPACK
 *
 * Piessens, R., de Doncker-Kapenga, E., Überhuber, C. W., & Kahaner, D. K. (2012). Quadpack: a subroutine package for automatic integration (Vol. 1). Springer Science & Business Media.
 */
public final class GaussKronrod extends Quadrature {
    private static final double[] D1MACH = { Double.MIN_VALUE, Double.MAX_VALUE, Math.pow(2.0, -52), Math.pow(2.0, -51),
	    Math.log(2.0) / Math.log(10.0) };

    private static final double EPMACH = D1MACH[4 - 1];
    private static final double UFLOW = D1MACH[1 - 1];
    private static final double OFLOW = D1MACH[2 - 1];
    ...
    private static final void dqk21(final Function<? super Double, Double> f, final double a, final double b,
	...
	for (j = 1; j <= 5; ++j) {
	    ...
	    fval1 = f.apply(centr - absc);
	    fval2 = f.apply(centr + absc);
	    fv1[jtw - 1] = fval1;
	    fv2[jtw - 1] = fval2;
	    fsum = fval1 + fval2;
	    resg += WG21[j - 1] * fsum;
    ...
    // *******************************************************************************
    private static void dqk15i(final Function<? super Double, Double> f, final double boun, final int inf,
    ...
    private static void dqagie(final Function<? super Double, Double> f, final double bound, final int inf,
	    final double epsabs, final double epsrel, final int limit, final double[] result, final double[] abserr,
	    final int[] neval, final int[] ier, final double[] alist, final double[] blist, final double[] rlist,
	    final double[] elist, final int[] iord, final int[] last) {
    ...
    private static void dqagi1(final Function<? super Double, Double> f, final double boun, final int inf,
	    final double epsabs, final double epsrel, final double[] result, final double[] abserr, final int[] neval,
	    final int[] ier, final int limit) {
